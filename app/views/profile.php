								...
								<tbody>
									...
									$sql = "SELECT o.transaction_code, o.purchase_date, s.name AS status, p.name AS payment_mode FROM orders o JOIN statuses s ON (o.status_id=s.id) JOIN payment_modes p ON (o.payment_mode_id=p.id) WHERE user_id = ". $user['id'];
                                        ...
                                        foreach($transactions as $transaction) { 
                                        	...
                                        	?>
                                          	<tr>
                                          		<td><?php echo $transaction['transaction_code'] ?></td> <!-- transaction code -->
                                          		<td><?php echo $transaction['purchase_date'] ?></td> <!-- purchase date -->
                                          		<td><?php echo $transaction['status'] ?></td> <!-- status -->
                                          		<td><?php echo $transaction['payment_mode'] ?></td> <!-- payment mode -->
                                          	</tr>
                                        <?php  }  
									?>
								</tbody>
